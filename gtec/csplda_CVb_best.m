function bestaccu = csplda_CVb_best(basename)
...
cspwinorig = [1 3.5];
cspwin = (cspwinorig .* P_C_S.SamplingFrequency) + P_C_S.PreTrigger;
...
for block = 1:length(blocklist)
    ...
    fprintf('There are %s trials in block %s\n',num2str(blocklength(block)),num2str(block));
...
targetwin = cspwinorig; %time window relative to stimulus onset within which to identify best classifier accuracy
...
%     
    num2use = 3;
%     [rhout, toout] = cspdistance_eig(CSP_O, goodchannels, num2use, CVrun, basename);
...
save(sprintf('%s_csptt.mat',char(basename)),'overall_accu','testaccu');
save(sprintf('%s_cspAllFilts.mat',char(basename)),'AllFilts');
