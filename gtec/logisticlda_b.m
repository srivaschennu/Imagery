...
%         trainlabels = classlabels(setdiff(1:length(P_C_S.TrialNumber),runs(run):runs(run+1)-1));
        ...
        for f = 1:size(features,3)
            r_pb(f) = fisherc(trainlabels,trainfeatures(:,f));
        end
        
        [~,sortidx] = sort(r_pb,'descend');
        numfeatures = round(size(trainfeatures,1)/5);
        usefeatures(t,sortidx(1:numfeatures)) = true;
        
        b = glmfit(trainfeatures(:,usefeatures(t,:)),trainlabels,'binomial','link','logit');
        WM(run,t,1:numfeatures+1) = b;

        trainres = glmval(squeeze(WM(run,t,1:numfeatures+1)),trainfeatures(:,usefeatures(t,:)),'logit');
        trainaccu(run,t) = (sum(~xor(trainres > 0.5, trainlabels > 0.5))/length(trainlabels)) * 100;
    end
...
%             testres = testfeatures * squeeze(WM(run,t,:));
            ...
            testaccu(run,t) = (sum(~xor(testres > 0.5, testlabels > 0.5))/length(testlabels)) * 100;
        end
...
WM = squeeze(WM);
WV = squeeze(WV);
save(sprintf('%s_loglda.mat',P_C_S.SubjectID),'WM','WV','bestwvidx','trainaccu','testaccu','finalaccu','bestaccu');
